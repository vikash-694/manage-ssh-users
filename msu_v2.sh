...
prompt_if_needed() {
  ...
  if [[ -z "$PUB_KEY" ]]; then
    ...
    if [[ -n "$PUB_KEY" && ! is_valid_pubkey "$PUB_KEY" ]]; then
...
apply_fixes() {
  ...
  if command -v restorecon &>/dev/null; then
    ...
    log_action "Ran restorecon on $HOMEDIR (if SELinux)"
  fi
  ...
  if getent group "$ADMIN_GROUP" >/dev/null; then
    ...
    log_action "Admin group $ADMIN_GROUP not present; skipping group add"
  fi
  ...
  if [[ "$GRANT_NOPASSWD" -eq 1 ]]; then
    ...
    if [[ ! -f "$SUDOERS_FILE" ]]; then
      run_or_dry "printf '%%%s ALL=(ALL) NOPASSWD: ALL\n' '$ADMIN_GROUP' > '$SUDOERS_FILE'"
      run_or_dry "chmod 440 '$SUDOERS_FILE'"
      if [[ "$DRY_RUN" -eq 0 ]]; then
        if ! visudo -cf "$SUDOERS_FILE"; then
          echo "ERROR: visudo check failed for $SUDOERS_FILE; removing file" >&2
      ...
      log_action "Passwordless sudo already present at $SUDOERS_FILE"
    fi
  fi
...
echo
echo "Done. Review actions in $LOG_FILE"
